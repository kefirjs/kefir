...
describe('last', function() {
  ...
  describe('stream', function() {
    ...
    return it('should handle events', function() {
      const a = stream();
      return expect(a.last()).toEmit([{error: 5}, {error: 6}, 3, '<end>'], () => send(a, [1, {error: 5}, {error: 6}, 2, 3, '<end>']));
    });
  });
  ...
  return describe('property', function() {
    ...
    return it('should handle events and current', function() {
      ...
      a = send(prop(), [{error: 0}]);
      return expect(a.last()).toEmit([{currentError: 0}, {error: 5}, 3, '<end>'], () => send(a, [2, {error: 5}, 3, '<end>']));
    });
  });
});
