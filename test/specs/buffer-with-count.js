...
describe('bufferWithCount', function() {
  describe('stream', function() {
    ...
    it('.bufferWithCount(1) should work correctly', function() {
      const a = stream()
      return expect(a.bufferWithCount(1)).toEmit([[1], [2], [3], [4], [5], '<end>'], () =>
    ...
    it('.bufferWithCount(2) should work correctly', function() {
      const a = stream()
      return expect(a.bufferWithCount(2)).toEmit([[1, 2], [3, 4], [5], '<end>'], () =>
        send(a, [1, 2, 3, 4, 5, '<end>'])
      )
    })
    ...
    it('.bufferWithCount(3) should work correctly', function() {
      const a = stream()
      return expect(a.bufferWithCount(3)).toEmit([[1, 2, 3], [4, 5], '<end>'], () => send(a, [1, 2, 3, 4, 5, '<end>']))
    })

    it('should not flush buffer on end if {flushOnEnd: false}', function() {
      const a = stream()
      return expect(a.bufferWithCount(3, {flushOnEnd: false})).toEmit([[1, 2, 3], '<end>'], () =>
        send(a, [1, 2, 3, 4, 5, '<end>'])
      )
    })
  ...
  return describe('property', function() {
    ...
    it('.bufferWithCount(1) should work correctly', function() {
      const a = send(prop(), [1])
      return expect(a.bufferWithCount(1)).toEmit([{current: [1]}, [2], [3], [4], [5], '<end>'], () =>
        send(a, [2, 3, 4, 5, '<end>'])
      )
    })
    ...
    it('.bufferWithCount(2) should work correctly', function() {
      const a = send(prop(), [1])
      return expect(a.bufferWithCount(2)).toEmit([[1, 2], [3, 4], [5], '<end>'], () => send(a, [2, 3, 4, 5, '<end>']))
    })

    it('.bufferWithCount(3) should work correctly', function() {
      const a = send(prop(), [1])
      return expect(a.bufferWithCount(3)).toEmit([[1, 2, 3], [4, 5], '<end>'], () => send(a, [2, 3, 4, 5, '<end>']))
    })
