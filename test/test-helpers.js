...
exports.withFakeTime = function(cb, reverseSimultaneous) {
  if (reverseSimultaneous == null) { reverseSimultaneous = false; }
...
exports.shakyTimeTest = function(testCb) {
  ...
  it('[shaky time test: normal run]', function() {
    const expectToEmitOverShakyTime = (stream, expectedLog, cb, timeLimit) => expect(stream).toEmitInTime(expectedLog, cb, timeLimit);
    return testCb(expectToEmitOverShakyTime);
  });

  return it('[shaky time test: reverse run]', function() {
    const expectToEmitOverShakyTime = (stream, expectedLog, cb, timeLimit) => expect(stream).toEmitInTime(expectedLog, cb, timeLimit, true);
    return testCb(expectToEmitOverShakyTime);
  });
};
...
beforeEach(function() {
  return this.addMatchers({
    toBeProperty() {
      this.message = function() { return `Expected ${this.actual.toString()} to be instance of Property`; };
      return this.actual instanceof Kefir.Property;
    },
    toBeStream() {
      this.message = function() { return `Expected ${this.actual.toString()} to be instance of Stream`; };
      return this.actual instanceof Kefir.Stream;
    },
    toBePool() {
      this.message = function() { return `Expected ${this.actual.toString()} to be instance of Pool`; };
      return this.actual instanceof Kefir.Pool;
    },

    toBeActive() { return this.actual._active; },
    ...
    errorsToFlow(source) {
      ...
      if (this.isNot) {
        ...
        this.message = () => `Expected errors to flow (i.e. to emit ${jasmine.pp(expectedLog)}, actually emitted ${jasmine.pp(log)})`;
        return this.env.equals_(expectedLog, log);
      }
    },

    toEmitInTime(expectedLog, cb, timeLimit, reverseSimultaneous) {
      if (timeLimit == null) { timeLimit = 10000; }
      if (reverseSimultaneous == null) { reverseSimultaneous = false; }
      let log = null;
    ...
    toActivate(...obss) {
      ...
      check("some activated at start", ((() => {
        const result = [];
        for (obs of Array.from(obss)) {           result.push(!obs._active);
        }
        return result;
      })()));

      exports.activate(this.actual);
      check(`some ${notNotStr}activated`, ((() => {
        const result1 = [];
        for (obs of Array.from(obss)) {           result1.push(obs._active);
        }
        return result1;
      })()));

      exports.deactivate(this.actual);
      check(`some ${notNotStr}deactivated`, ((() => {
        const result2 = [];
        for (obs of Array.from(obss)) {           result2.push(!obs._active);
        }
        return result2;
      })()));

      exports.activate(this.actual);
      check(`some ${notNotStr}activated at second try`, ((() => {
        const result3 = [];
        for (obs of Array.from(obss)) {           result3.push(obs._active);
        }
        return result3;
      })()));

      exports.deactivate(this.actual);
      check(`some ${notNotStr}deactivated at second try`, ((() => {
        const result4 = [];
        for (obs of Array.from(obss)) {           result4.push(!obs._active);
        }
        return result4;
      })()));
      ...
      this.message = function() {
        ...
        const obssNames = ((() => {
          const result6 = [];
          for (obs of Array.from(obss)) {             result6.push(obs.toString());
          }
          return result6;
        })()).join(', ');
