...
describe('filter', function() {
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () =>
      expect(send(stream(), ['<end>']).filter(function() {})).toEmit(['<end:current>']))

    it('should handle events', function() {
      const a = stream()
      return expect(a.filter(x => x > 3)).toEmit([4, 5, {error: 7}, 6, '<end>'], () =>
    ...
    return it('shoud use id as default predicate', function() {
      const a = stream()
      return expect(a.filter()).toEmit([4, 5, {error: 7}, 6, '<end>'], () =>
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () =>
      expect(send(prop(), ['<end>']).filter(function() {})).toEmit(['<end:current>']))
    ...
    it('should handle events and current', function() {
      ...
      a = send(prop(), [{error: 0}])
      return expect(a.filter(x => x > 2)).toEmit([{currentError: 0}, 4, {error: 7}, 3, '<end>'], () =>
        send(a, [4, {error: 7}, 3, 2, 1, '<end>'])
      )
    })
    ...
    return it('shoud use id as default predicate', function() {
      ...
      a = send(prop(), [1])
      return expect(a.filter()).toEmit([{current: 1}, 4, {error: -2}, 5, 6, '<end>'], () =>
