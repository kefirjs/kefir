...
describe('bufferWhile', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).bufferWhile(not3)).toEmit(['<end:current>']));

    it('should work correctly', function() {
      const a = stream();
      return expect(a.bufferWhile(not3)).toEmit([
        [3],
        [1, 2, 3],
        [4, 3],
        [3],
    ...
    it('should not flush buffer on end if {flushOnEnd: false}', function() {
      const a = stream();
      return expect(a.bufferWhile(not3, {flushOnEnd: false})).toEmit([
        [3],
        [1, 2, 3],
        [4, 3],
        [3],
