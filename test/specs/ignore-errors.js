...
describe('ignoreErrors', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).ignoreErrors()).toEmit(['<end:current>']));
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).ignoreErrors()).toEmit(['<end:current>']));
    ...
    return it('should handle events and current', function() {
      ...
      a = send(prop(), [1]);
      return expect(a.ignoreErrors()).toEmit([{current: 1}, 2, 3, '<end>'], () => send(a, [2, {error: -2}, 3, {error: -3}, '<end>']));
    });
  });
});
