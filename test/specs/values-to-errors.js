...
describe('valuesToErrors', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).valuesToErrors(function() {})).toEmit(['<end:current>']));
    ...
    return it('default handler should convert all values', function() {
      const a = stream();
      return expect(a.valuesToErrors()).toEmit([{error: 1}, {error: -2}, {error: -3}, {error: -4}, {error: 5}, '<end>'], () =>
        send(a, [
          1,
          -2,
          {error: -3},
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).valuesToErrors(function() {})).toEmit(['<end:current>']));

    it('should handle events', function() {
      const a = send(prop(), [1]);
      return expect(a.valuesToErrors(handler)).toEmit([{current: 1}, {error: -6}, {error: -3}, {error: -12}, 5, '<end>'], () =>
        send(a, [
          -2,
          {error: -3},
          -4,
