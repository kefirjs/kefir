...
describe('scan', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).scan(noop, 0)).toEmit([{current: 0}, '<end:current>']));
    ...
    it('should fall back to the seed after error, if seed specified', function() {
      const a = stream();
      return expect(a.scan(((res, x) => res + x), 'seed')).toEmit([{current: 'seed'}, 'seed1', {error: 'err'}, 'seed2', 'seed23', '<end>'], () => send(a, [1, {error: 'err'}, 2, 3, '<end>']));
    });

    return it('should use next value after error as seed, if seed not specified', function() {
      const a = stream();
      return expect(a.scan(() => 'abc')).toEmit([1, 'abc', {error: 'err'}, 3, 'abc', '<end>'], () => send(a, [1, 2, {error: 'err'}, 3, 4, '<end>']));
    });
  });
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).scan(noop, 0)).toEmit([{current: 0}, '<end:current>']));
