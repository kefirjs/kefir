...
describe('takeErrors', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).takeErrors(3)).toEmit(['<end:current>']));

    it('should be ended if `n` is 0', () => expect(stream().takeErrors(0)).toEmit(['<end:current>']));

    it('should handle events (less than `n`)', function() {
      const a = stream();
      return expect(a.takeErrors(3)).toEmit([{error: 1}, {error: 2}, '<end>'], () => send(a, [{error: 1}, {error: 2}, '<end>']));
    });

    it('should handle events (more than `n`)', function() {
      const a = stream();
      return expect(a.takeErrors(3)).toEmit([{error: 1}, {error: 2}, {error: 3}, '<end>'], () => send(a, [{error: 1}, {error: 2}, {error: 3}, {error: 4}, {error: 5}, '<end>']));
    });
    ...
    return it('should emit once on circular dependency', function() {
      ...
      return expect(b).toEmit([{error: 2}, '<end>'], () => send(a, [{error: 1}, {error: 2}, {error: 3}, {error: 4}, {error: 5}]));
    });
  });
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).takeErrors(3)).toEmit(['<end:current>']));
    ...
    it('should handle events and current (less than `n`)', function() {
      ...
      return expect(a.takeErrors(3)).toEmit([{currentError: 1}, {error: 2}, '<end>'], () => send(a, [{error: 2}, '<end>']));
    });

    it('should handle events and current (more than `n`)', function() {
      const a = send(prop(), [{error: 1}]);
      return expect(a.takeErrors(3)).toEmit([{currentError: 1}, {error: 2}, {error: 3}, '<end>'], () => send(a, [{error: 2}, {error: 3}, {error: 4}, {error: 5}, '<end>']));
    });

    it('should work correctly with .constant', () => expect(Kefir.constantError(1).takeErrors(1)).toEmit([{currentError: 1}, '<end:current>']));
