  ...
  describe(`with ${name} implementation`, function() {
    ...
    describe('stream', function() {
      ...
      it('`filter` should work', function() {
        const a = stream();
        return expect(a.transduce(t.filter(x => (x % 2) === 0))).toEmit([2, 4, '<end>'], () => send(a, [1, 2, 3, 4, '<end>']));
      });
      ...
      if (t.partitionAll) {
        it('`partitionAll` should work', function() {
          ...
          a = stream();
          return expect(a.transduce(t.partitionAll(2))).toEmit([[1, 2], [3], '<end>'], () => send(a, [1, 2, 3, '<end>']));
        });
    ...
    return describe('property', function() {
      ...
      it('`map` should work', function() {
        const a = send(prop(), [1]);
        return expect(a.transduce(t.map(x => x * 2))).toEmit([{current: 2}, 4, 6, '<end>'], () => send(a, [2, 3, '<end>']));
      });
      ...
      it('`filter` should work', function() {
        ...
        a = send(prop(), [2]);
        return expect(a.transduce(t.filter(x => (x % 2) === 0))).toEmit([{current: 2}, 4, '<end>'], () => send(a, [1, 3, 4, '<end>']));
      });
...
describe('transduce', function() {
  ...
  describe('with `noop` transducer', function() {
    ...
    describe('stream', function() {
      ...
      it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).transduce(noop)).toEmit(['<end:current>']));

      return it('should handle events', function() {
        const a = stream();
        return expect(a.transduce(noop)).toEmit([1, 2, {error: 4}, 3, '<end>'], () => send(a, [1, 2, {error: 4}, 3, '<end>']));
      });
    });
    ...
    return describe('property', function() {
      ...
      it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).transduce(noop)).toEmit(['<end:current>']));

      return it('should handle events and current', function() {
        let a = send(prop(), [1]);
        expect(a.transduce(noop)).toEmit([{current: 1}, 2, {error: 4}, 3, '<end>'], () => send(a, [2, {error: 4}, 3, '<end>']));
        a = send(prop(), [{error: 0}]);
        return expect(a.transduce(noop)).toEmit([{currentError: 0}, 2, {error: 4}, 3, '<end>'], () => send(a, [2, {error: 4}, 3, '<end>']));
      });
    });
  });
