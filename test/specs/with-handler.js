...
describe('withHandler', function() {
  ...
  describe('stream', function() {
    ...
    it('should not be ended if source was ended (by default)', () =>
      expect(send(stream(), ['<end>']).withHandler(function() {})).toEmit([]))
    ...
    it('should handle events (with `duplicate` handler)', function() {
      const a = stream()
      return expect(a.withHandler(duplicate)).toEmit([1, 1, {error: 3}, {error: 3}, 2, 2, '<end>'], () =>
    ...
    return it('should support emitter.emitEvent', function() {
      const a = stream()
      return expect(a.withHandler(emitEventMirror)).toEmit([1, {error: 3}, 2, '<end>'], () =>
        send(a, [1, {error: 3}, 2, '<end>'])
      )
    })
  })
  ...
  return describe('property', function() {
    ...
    it('should not be ended if source was ended (by default)', () =>
      expect(send(prop(), ['<end>']).withHandler(function() {})).toEmit([]))
    ...
    it('should handle events and current (with `duplicate` handler)', function() {
      ...
      return expect(a.withHandler(duplicate)).toEmit(
        [{currentError: 0}, 2, 2, {error: 4}, {error: 4}, 3, 3, '<end>'],
        () => send(a, [2, {error: 4}, 3, '<end>'])
      )
    })
