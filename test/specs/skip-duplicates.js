...
describe('skipDuplicates', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).skipDuplicates()).toEmit(['<end:current>']));
    ...
    it('should handle events (custom comparator)', function() {
      const a = stream();
      return expect(a.skipDuplicates(roundlyEqual)).toEmit([1, 2, 3.8, '<end>'], () => send(a, [1, 1.1, 2, 3.8, 4, '<end>']));
    });
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).skipDuplicates()).toEmit(['<end:current>']));
    ...
    it('should handle events and current (custom comparator)', function() {
      const a = send(prop(), [1]);
      return expect(a.skipDuplicates(roundlyEqual)).toEmit([{current: 1}, 2, 3, '<end>'], () => send(a, [1.1, 1.2, 2, 3, 3.2, '<end>']));
    });
