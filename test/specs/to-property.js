...
describe('toProperty', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).toProperty()).toEmit(['<end:current>']));

    it('should be ended if source was ended (with current)', () => expect(send(stream(), ['<end>']).toProperty(() => 0)).toEmit([{current: 0}, '<end:current>']));
  ...
  return describe('property', function() {
    ...
    return it('if original property has no current, and .toProperty called with no arguments, then result should have no current', () => expect(prop().toProperty()).toEmit([]));
});
});
