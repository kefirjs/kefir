...
describe('diff', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).diff(noop, 0)).toEmit(['<end:current>']));
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).diff(noop, 0)).toEmit(['<end:current>']));
    ...
    it('works without fn argument', function() {
      const a = send(prop(), [1]);
      return expect(a.diff(null, 0)).toEmit([{current: [0,1]}, [1,3], [3,6], '<end>'], () => send(a, [3, 6, '<end>']));
    });
