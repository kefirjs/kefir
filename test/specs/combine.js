...
describe('combine', function() {
  describe('array', function() {
    ...
    it('should handle events and current from observables', function() {
      ...
      expect(Kefir.combine([a, b, c])).toEmit(
        [[1, 0, 2], [1, 3, 2], [1, 4, 2], [1, 4, 5], [1, 4, 6], '<end>'],
        function() {
          send(a, [1])
          send(c, [2])
          send(b, [3])
    ...
    return describe('sampledBy functionality (3 arity combine)', function() {
      ...
      it('should accept optional combinator function', function() {
        ...
        const c = stream()
        const d = stream()
        return expect(Kefir.combine([c, d], [a, b], join)).toEmit(
          ['2+3+1+0', '5+3+1+4', '6+3+1+4', '6+7+1+4', '<end>'],
          function() {
            send(a, [1])
            send(c, [2])
  ...
  describe('object', function() {
    ...
    it('should accept optional combinator function', function() {
      ...
      return expect(Kefir.combine({a, b, c}, join)).toEmit(
        ['1+0+2', '1+3+2', '1+4+2', '1+4+5', '1+4+6', '<end>'],
        function() {
          send(a, [1])
          send(c, [2])
    ...
    return describe('sampledBy functionality (3 arity combine)', function() {
      ...
      it('should work nice for emitating atomic updates', function() {
        ...
        const b = a.map(x => x + 2)
        const c = a.map(x => x * 2)
        return expect(Kefir.combine({b}, {c})).toEmit([{b: 3, c: 2}, {b: 4, c: 4}, {b: 5, c: 6}], () =>
          send(a, [1, 2, 3])
        )
      })
      ...
      return it('should prefer active keys over passive keys', function() {
        ...
        return expect(Kefir.combine({a, b}, {a: _a})).toEmit([{a: 1, b: 4}, {a: 2, b: 4}, {a: 3, b: 4}], function() {
          send(_a, [-1])
          send(a, [1])
          send(b, [4])
          send(_a, [-2])
