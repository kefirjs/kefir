...
describe('flatten', function() {
  ...
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () => expect(send(stream(), ['<end>']).flatten(function() {})).toEmit(['<end:current>']));

    it('should handle events', function() {
      const a = stream();
      return expect(
        a.flatten(function(x) { if (x > 1) { return __range__(1, x, true); } else { return []; } })
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () => expect(send(prop(), ['<end>']).flatten(function() {})).toEmit(['<end:current>']));
    ...
    it('should handle current correctly', function() {
      ...
      return expect(
        send(prop(), [{error: 0}]).flatten(function() {})
      ).toEmit([{currentError: 0}]);
  });

    return it('should handle multiple currents correctly', () =>
      expect(
        send(prop(), [2]).flatten(x => __range__(1, x, true))
