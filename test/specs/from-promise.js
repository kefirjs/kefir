...
describe('fromPromise', function() {
  ...
  it('should call `property.then` on first activation, and only on first', function() {
    let count = 0;
    const s = Kefir.fromPromise({then() { return count++; }});
    expect(count).toBe(0);
    activate(s);
    expect(count).toBe(1);
    deactivate(s);
  ...
  it('should call `property.done`', function() {
    let count = 0;
    const s = Kefir.fromPromise({then() { return this; }, done() { return count++; }});
    expect(count).toBe(0);
    activate(s);
    expect(count).toBe(1);
    deactivate(s);
  ...
  it('... with fulfilledSync property', () => expect(Kefir.fromPromise(fulfilledSync)).toEmit([{ current : 1 }, '<end:current>']));

  it('... with failedSync property', () => expect(Kefir.fromPromise(failedSync)).toEmit([{ currentError : 1 }, '<end:current>']));
