...
describe '.flatMapLatest()', ->
  ...
  it 'if sorce ended should end when latest sub-source ends', ->
    p = prop()
    mapped = p.flatMapLatest()
    mapped.on('value', ->) # in order to sub sources to be added prop should be active
  ...
  it 'if orig property has current *value*, and it is property that in turn has current *value*, that current *value* should became current *value* of result property', ->
    expect(  prop(prop(1)).flatMapLatest()  ).toHasValue(1)
  ...
  it 'if orig property has current *error*, and it is property that in turn has current *error*, that current *error* should became current *error* of result property', ->
    expect(  prop(prop(null, 1)).flatMapLatest()  ).toHasError(1)

  it 'should handle initial *error*', ->
    expect(  prop(null, 1).flatMapLatest()  ).toHasError(1)
  ...
  it 'should pass all values/errors from only latest sub sources', ->
    ...
    send(p2, 'value', 3)
    send(p2, 'error', 'e3')
    expect(state).toEqual({values:[1,3],errors:['e1','e3'],ended:false})
  ...
  it 'allows to pass optional mapFn', ->
    ...
    send(p1, 'value', 1)
    send(p1, 'error', 'e1')
    expect(state).toEqual({values:[0,1],errors:['e0','e1'],ended:false})
    ...
    send(p2, 'value', 3)
    send(p2, 'error', 'e3')
    expect(state).toEqual({values:[0,1,3],errors:['e0','e1','e3'],ended:false})
