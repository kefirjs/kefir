...
describe('mapErrors', function() {
  describe('stream', function() {
    ...
    it('should be ended if source was ended', () =>
      expect(send(stream(), ['<end>']).mapErrors(function() {})).toEmit(['<end:current>']))

    return it('should handle events', function() {
      const a = stream()
      return expect(a.mapErrors(x => x * 2)).toEmit([1, {error: -2}, 2, {error: -4}, '<end>'], () =>
  ...
  return describe('property', function() {
    ...
    it('should be ended if source was ended', () =>
      expect(send(prop(), ['<end>']).mapErrors(function() {})).toEmit(['<end:current>']))
    ...
    return it('should handle events and current', function() {
      ...
      a = send(prop(), [{error: -1}])
      return expect(a.mapErrors(x => x * 2)).toEmit([{currentError: -2}, 2, {error: -4}, 3, {error: -6}, '<end>'], () =>
        send(a, [2, {error: -2}, 3, {error: -3}, '<end>'])
      )
    })
  })
